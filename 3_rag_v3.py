...
@traceable(name="load_pdf", tags=['load', 'load_pdf', 'pdf'])
def load_pdf():
...
@traceable(name="split_documents", tags=['split', 'document'])
def split_docs(docs):
...
@traceable(name="build_vector_store", tags=['vector store', 'store', 'db'])
...
@traceable(name="setup_pipeline", tags=['setup', 'pipeline'], metadata={'embedding_model': "ChatOpenAI"})
def setup_pipeline():
...
def setup_pipeline_and_query(q):
    ...
    config: RunnableConfig = {
        "run_name": "Rag Based App Version 3",
